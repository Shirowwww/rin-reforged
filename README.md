...
- **Finds the current version.** A **Releases** panel lists everything ever
  posted in a topic — release, update, repack, pre-installed build, crack,
  hypervisor crack, clean Steam files, online fix, DLC unlocker, trainer,
  savegame, language pack, tool — with its version, who posted it, when, and
  which page it is on. It lists what a post is *offering*: a question about a
  release, a link to a store page or a patch note, a pasted log and somebody
  explaining what you should do about your copy are not releases, and a
  release that hides its mirrors inside a spoiler is. The highest version
  anybody posted **of the game** is called out at the top — not the trainer's,
  not the emulator's, not the number in a reply that dropped a digit.
