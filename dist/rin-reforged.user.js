 ...
 *
 * The selector is this shape because the rule it has to beat is
 * \`html[data-rr] div.forumrules[data-rr-rules] h3\`, which is three
 * classes and three elements. A bare \`.rr-rules__toggle > h3\` is three
 * and two and loses on the element count; adding \`button\` only ties,
 * and a tie is decided by which rule is written last, which is not
 * something to depend on. \`:is()\` takes the specificity of its
 * strongest argument — \`p.rules\` — so this is four and three and wins
 * outright, whichever of the four it is matching.
 *
 * It lost silently before, and the 12px bottom margin it left on the
 * label is what pushed the chevron 6px below the words it points at:
 * a flex row centres margin boxes, so a margin under one item lifts
 * that item and drops every other. */
